<?php
$allowedtypes= array("image/jpg","image/jpeg","image/png","image/webp");
...
<?php
$allowedtypes = array("image/jpg", "image/jpeg", "image/png", "image/webp"); // Agregar WebP a la lista
$descripcion = !empty($_POST['descripcion']) ? $_POST['descripcion'] : '';
$material = !empty($_POST['material']) ? $_POST['material'] : '';
$color = !empty($_POST['color']) ? $_POST['color'] : '';
        ...
        if (isset($mfiles)) {
            ...
            for ($j = 0; $j < $x; $j++) {
                ...
                if (!in_array($filetype, $allowedtypes)) {
                    ...
                    $extension = ".png"; // Establecer la extensión por defecto como PNG
                    
                    // Si el archivo es WebP, convertirlo a PNG o JPG
                    if ($filetype == "image/webp") {
                        $img = imagecreatefromwebp($mfiles["tmp_name"][$j]);
                        $extension = ".png"; // Cambiar a PNG
                        $filelocation = "../../imagenes/Producto" . $prodnum . "/img" . $imgnum . $extension;
                        imagepng($img, $filelocation); // Guardar la imagen convertida
                        imagedestroy($img);
                    } else {
                        // Para otros tipos de imagen, como JPG o PNG, simplemente movemos el archivo
                        $filelocation = "../../imagenes/Producto" . $prodnum . "/img" . $imgnum . $extension;
                        move_uploaded_file($mfiles["tmp_name"][$j], $filelocation);
                    }
